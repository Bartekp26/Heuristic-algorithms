// function-info.js - Obsługa informacji o funkcjach testowych i heatmapy
...
function evaluateRastrigin(x, y) {
    const n = 2;
    const A = 10;
    const val = A * n + (x*x - A*Math.cos(2*Math.PI*x)) + (y*y - A*Math.cos(2*Math.PI*y));
    return val;
}
...
function updateFunctionInfo(functionName) {
    ...
    if (descriptionDiv) {
        descriptionDiv.innerHTML = `
            ...
            <p>Zakres: ${details.range}</p>
            <p>Minimum globalne: ${details.minimum}</p>
            <p>Charakterystyka: ${details.description}</p>
        `;
    }
